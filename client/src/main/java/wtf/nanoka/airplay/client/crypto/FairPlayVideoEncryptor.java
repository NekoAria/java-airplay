package wtf.nanoka.airplay.client.crypto;
...
public class FairPlayVideoEncryptor {
    ...
    public void encrypt(byte[] video) throws Exception {
        ...
        if (nextEncryptCount > 0) {
            int availableKeyBytes = nextEncryptCount;
            int consumed = Math.min(availableKeyBytes, video.length);
            for (int i = 0; i < consumed; i++) {
                video[i] = (byte) (video[i] ^ og[(16 - availableKeyBytes) + i]);
            }
            nextEncryptCount -= consumed;
            offset = consumed;
        ...
        int encryptlen = ((video.length - offset) / 16) * 16;
        aesCtrEncrypt.update(video, offset, encryptlen, video, offset);

        int restlen = (video.length - offset) % 16;
