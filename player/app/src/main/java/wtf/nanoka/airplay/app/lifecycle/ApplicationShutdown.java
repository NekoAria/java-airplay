...
public final class ApplicationShutdown {
    ...
    private final Consumer<Runnable> uiPreparationScheduler;
    ...
    public ApplicationShutdown(ApplicationContext applicationContext) {
        this(closeAction(applicationContext),
                System::exit,
    ...
    ApplicationShutdown(Runnable closeApplication,
        ...
        this(closeApplication, processExit, forcedExit, timeoutMillis, threadFactory,
                Runnable::run);
    }
    ...
    ApplicationShutdown(Runnable closeApplication,
                        ...
                        long timeoutMillis,
                        ThreadFactory threadFactory,
                        Consumer<Runnable> uiPreparationScheduler) {
        this.uiPreparationScheduler = Objects.requireNonNull(uiPreparationScheduler, "uiPreparationScheduler");
    ...
    public void requestQuit() {
        ...
        try {
            ...
            CountDownLatch uiPrepared = beginUiPreparation();
            startGate = new CountDownLatch(1);
            startShutdownThreads(startGate, uiPrepared);
    ...
    private CountDownLatch beginUiPreparation() {
        CountDownLatch uiPrepared = new CountDownLatch(1);
        try {
            uiPreparationScheduler.accept(uiPrepared::countDown);
        } catch (Throwable error) {
            log.warn("Unable to schedule application windows for hiding before shutdown", error);
            uiPrepared.countDown();
        }
        return uiPrepared;
    }

    static void hideApplicationWindows(Runnable completion) {
        Runnable hideWindowsAndComplete = () -> {
            try {
                for (Window window : Window.getWindows()) {
                    window.setVisible(false);
                }
            } catch (Throwable error) {
                log.warn("Unable to hide application windows before shutdown", error);
            } finally {
                completion.run();
            }
        };
        if (EventQueue.isDispatchThread()) {
            hideWindowsAndComplete.run();
            return;
        }
        EventQueue.invokeLater(hideWindowsAndComplete);
    }

    private void startShutdownThreads(CountDownLatch startGate, CountDownLatch uiPrepared) {
        Thread shutdownWorker = createThread("java-airplay-shutdown", false, () -> {
            awaitStart(startGate);
            awaitStart(uiPrepared);
            runShutdown();
        });
