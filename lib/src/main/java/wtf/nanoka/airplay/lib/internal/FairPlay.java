...
public class FairPlay {
    ...
    private boolean keyMessageReceived;

    public void fairPlaySetup(InputStream request, OutputStream response) throws IOException {
        byte[] data = request.readAllBytes();
        if (data.length != 16 && data.length != 164) {
            throw new IllegalArgumentException("FairPlay setup payload must contain 16 or 164 bytes");
        }
        if (data[4] != 3) {
            throw new IllegalArgumentException("FairPlay version " + data[4] + " is not supported");
        }
        if (data.length == 16) {
            int mode = data[14] & 0xff;
            byte[][] replyMessage = {
            ...
            if (mode >= replyMessage.length) {
                throw new IllegalArgumentException("Unsupported FairPlay mode: " + mode);
            }
            response.write(replyMessage[mode]);
        } else if (data.length == 164) {
            System.arraycopy(data, 0, keyMsg, 0, 164);
            keyMessageReceived = true;
    ...
    public byte[] decryptAesKey(byte[] key) {
        if (!keyMessageReceived) {
            throw new IllegalStateException("FairPlay key message has not been received");
        }
        if (key == null || key.length != 72) {
            throw new IllegalArgumentException("Encrypted FairPlay AES key must contain 72 bytes");
        }
        byte[] aesKey = new byte[16];
        omgHax.decryptAesKey(keyMsg, key, aesKey);
        log.debug("FairPlay AES key decrypted");
        return aesKey;
    }
}
