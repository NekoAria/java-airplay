...
public class Pairing {
    ...
    public Pairing(KeyPair keyPair) {
    ...
    public void pairVerify(InputStream request, OutputStream response) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, InvalidKeyException, SignatureException, BadPaddingException, IllegalBlockSizeException, IOException {
        byte[] header = request.readNBytes(4);
        if (header.length != 4) {
            throw new IllegalArgumentException("Pair verify header must contain 4 bytes");
        }
        int flag = header[0] & 0xff;
        if (flag > 0) {
            pairVerified = false;
            ecdhTheirs = readExactly(request, 32, "peer Curve25519 key");
            edTheirs = readExactly(request, 32, "peer Ed25519 key");
            ...
            log.debug("Pair verify shared secret established");
            ...
            if (response == null) {
                throw new IllegalArgumentException("Pair verify response stream is required for the first step");
            }
            response.write(responseContent);
        } else {
            if (ecdhSecret == null || ecdhTheirs == null || ecdhOurs == null || edTheirs == null) {
                throw new IllegalStateException("Pair verify second step received before the first step");
            }
            byte[] signature = readExactly(request, 64, "encrypted signature");
    ...
    private byte[] readExactly(InputStream input, int length, String field) throws IOException {
